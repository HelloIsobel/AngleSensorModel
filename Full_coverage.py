...
if __name__ == "__main__":
    ...
    batch = 10  # TODO  表示第几个变化边界点，该点之前均为准备状态
    ...
    # 设置 workingFlag 初始状态
    num00 = np.argwhere(abs(diff[0:num]) > T_upSize)
    if diff[num00[0]] < 0:
        workingFlag = h
    else:
        workingFlag = -h
    ...
    # 保存到excel
    excel_name = "result/" + "Full_" + csvname + "_batch" + str(batch) + "_minup" + str(minup) + "_T_upSize" + str(
        T_upSize) + ".xls"
    sheet_name = "patameters"
    title = ["angle", "state", "diff"]
    # 新建表格
    excel_int(excel_name, sheet_name)
    # 写入表头
    excel_write_title(excel_name, title)
    # 写入数据
    save_data = [angle, state, diff]
